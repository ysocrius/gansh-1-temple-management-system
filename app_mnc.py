...
# IPs that can bypass maintenance mode (add your own IP here for testing)
ALLOWED_IPS = [
    '127.0.0.1',
...
# Add maintenance mode middleware - MUST COME BEFORE OTHER MIDDLEWARE
@app.before_request
def check_for_maintenance():
    ...
    # Skip maintenance check for allowed IPs
    if request.remote_addr in ALLOWED_IPS:
        logger.debug(f"Maintenance bypass for IP: {request.remote_addr}")
        return None
    
    # Skip maintenance check for specified paths
    if any(request.path.startswith(path) for path in BYPASS_PATHS):
        logger.debug(f"Maintenance bypass for path: {request.path}")
        return None
    
    # If maintenance mode is on and the request isn't exempt, show maintenance page
    if MAINTENANCE_MODE:
        logger.debug(f"Showing maintenance page for: {request.path}")
        return render_template('maintenance.html'), 503  # 503 Service Unavailable

# Add maintenance toggle route
@app.route('/toggle-maintenance')
def toggle_maintenance():
    global MAINTENANCE_MODE
    
    # Only allow from allowed IPs
    if request.remote_addr not in ALLOWED_IPS:
        logger.warning(f"Unauthorized maintenance toggle attempt from IP: {request.remote_addr}")
        return "Unauthorized", 403
    
    # Toggle maintenance mode
    MAINTENANCE_MODE = not MAINTENANCE_MODE
    logger.info(f"Maintenance mode toggled to: {MAINTENANCE_MODE}")
    
    return f"Maintenance mode is now {'ON' if MAINTENANCE_MODE else 'OFF'}"
...
def check_auth_middleware():
    ...
    public_paths = [
        ...
        '/e-hundi',
        '/admin/login',
        '/toggle-maintenance'    # Allow maintenance toggle access
    ]
